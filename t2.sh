...
add-apt-repository ppa:openjdk-r/ppa -y && apt-get update && apt-get install -y libxp6 openjdk-8-jdk && update-alternatives --config java
...
#docker container commit -m "java python3"
